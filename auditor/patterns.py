...
PATTERNS: dict[str, list[Pattern]] = {
    ...
    "high": [
        {
            "id": "DATA_LEAK_001",
            "name": "Sensitive data on stdout",
            "regex": r"(base64.*\|\s*(cat|echo|printf|tee)|(scrot|screenshot|capture).*base64|base64\s+(--wrap|--decode|-d|-w)?\s*/tmp/)",
            "description": "Base64-encoded screenshots or captures piped to stdout — may leak tokens, passwords, or PII.",
        },
        {
            "id": "SANDBOX_DIS_001",
            "name": "Security sandbox disabled",
            "regex": r"(--no-sandbox|--disable-web-security|--allow-running-insecure-content|sandbox\s*=\s*(false|0|off|no)|--disable-setuid-sandbox)",
            "description": "Explicitly disabling security sandbox boundaries.",
        },
        {
            "id": "PROMPT_INJ_001",
        ...
        },
        {
            ...
            "regex": r"chmod\s+[0-7]*[2467][0-7]{3}\s|chmod\s+[ug]\+s\s",
            "description": "Setting setuid or setgid bits on a file (4xxx/2xxx/6xxx octal or u+s/g+s).",
        },
        {
            "id": "SKILL_ESCAPE_001",
    ...
    "medium": [
        ...
        {
            "id": "DOS_INPUT_001",
            "name": "Unbounded input parameter in loop",
            "regex": r"(for\s+\w+\s+in\s+\$\(seq\s+\$|while\s.*\$\{?\w*(count|num|iter|repeat|times)\}?|repeat\s+\$)",
            "description": "Loop with user-controlled iteration count and no upper bound — DoS risk via resource exhaustion.",
        },
        {
            "id": "DEP_MISSING_001",
            "name": "Undocumented binary dependency",
            "regex": r"(?<!\w)(bc|jq|convert|magick|ffmpeg|ffprobe|socat|nmap|imagemagick|pandoc|wkhtmltopdf)\s+",
            "description": "Uses external binary that may not be documented in requirements — silent failure in minimal environments.",
        },
        {
            "id": "TMPFILE_SEC_001",
            "name": "Insecure temp file handling",
            "regex": r">\s*/tmp/[a-zA-Z][a-zA-Z0-9_.-]*(?!.*mktemp)",
            "description": "Writing to /tmp with predictable filename without mktemp — symlink attack and TOCTOU risk.",
        },
        {
            "id": "SUDO_001",
            "name": "Sudo usage",
